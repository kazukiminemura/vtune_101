...
結果はVTune Amplifierで開かれ、アプリケーションパフォーマンスの概要を示す「サマリー」ウィンドウが表示されます。    
"heart_demo"はMPI並列アプリケーションのため、「サマリー」ウィンドウには通常のメトリクスに加えてMPIの不均衡情報や、実行のクリティカルパスにおけるMPIランクの詳細が示されます。    
